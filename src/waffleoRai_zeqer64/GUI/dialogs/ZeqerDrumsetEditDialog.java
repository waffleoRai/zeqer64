...
public class ZeqerDrumsetEditDialog extends JDialog{
...
private static final long serialVersionUID = 6232066596448455202L;
	...
	private void initGUI(){
		...
		regionEnable = globalEnable.newChild();
		...
		gbl_pnlRegData.columnWidths = new int[]{0, 50, 70, 0};
		gbl_pnlRegData.rowHeights = new int[]{0, 0, 0, 0, 0};
		gbl_pnlRegData.columnWeights = new double[]{0.0, 1.0, 0.0, Double.MIN_VALUE};
		gbl_pnlRegData.rowWeights = new double[]{0.0, 0.0, 0.0, 0.0, Double.MIN_VALUE};
		...
		JButton btnRel = new JButton("Edit Release...");
		GridBagConstraints gbc_btnRel = new GridBagConstraints();
		gbc_btnRel.insets = new Insets(0, 0, 5, 0);
		gbc_btnRel.gridx = 2;
		gbc_btnRel.gridy = 0;
		pnlRegData.add(btnRel, gbc_btnRel);
		regionEnable.addComponent("btnRel", btnRel);
		btnRel.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {btnRelCallback();}
		});
		...
		gbc_lblPan.insets = new Insets(0, 0, 0, 5);
		gbc_lblPan.gridx = 0;
		gbc_lblPan.gridy = 3;
		...
		gbc_sldPan.insets = new Insets(0, 0, 0, 5);
		gbc_sldPan.gridx = 1;
		gbc_sldPan.gridy = 3;
		...
		gbc_sldFineTune.insets = new Insets(0, 0, 5, 5);
		gbc_sldFineTune.gridx = 1;
		gbc_sldFineTune.gridy = 2;
		...
		gbc_lblFineAmt.insets = new Insets(0, 0, 5, 0);
		...
		cgEditable.addComponent("btnApply", btnApply);
		...
		cgEditable.addComponent("btnInsert", btnInsert);
		...
		cgEditable.addComponent("btnSave", btnSave);
	...
	public void loadDataIntoDrum(ZeqerPercPreset drum){
		int rcount = regions.size();
		drum.clearAndReallocRegions(rcount);
		...
		for(DrumRegion r : regions){
			...
			drum.setRegion(i++, treg);
		}
	}
	
	public boolean getExitSelection(){
			...
			for(int i = 0; i < rcount; i++){
				...
				if(oreg != null){
					...
					if(drumdata != null){
						...
						treg.release = Byte.toUnsignedInt(drumdata.getDecay());
						treg.sample = drumdata.getSample();
						treg.unityKey = drumdata.getRootKey();
						treg.fineTune = drumdata.getFineTune();
					}
				}
			}
			
			tempTags = drum.getAllTags();
			
			String setname = drum.getName();
			if(setname == null){
				setname = String.format("PERC_%08x", drum.getUID());
			}
	...
	private void updatePanLabel(int value){
		if(value == 64){
			...
			lblPanSetting.setText(String.format("%.1f", perc) + "% Left");
		}
		else{
			//Right
			int amt = value - 64;
			double perc = ((double)amt / 64.0) * 100.0;
			lblPanSetting.setText(String.format("%.1f", perc) + "% Right");
		}
	...
	private void updateRegionList(){
		...
		//TODO If no region selected here, disable the region edit area
		loadRegionInfoToGUI(sel);
		reenable();
	}
	...
	private int applyGUIInfoToRegion(){
		...
		selectedRegion.release = tempRelease;
